...
class Rows implements SqlRows<Record<string, null | number | string | Uint8Array>> {
  [key: number]: never;
  ...
  constructor(firstRow: SqlValue[], rowStreamId: number) {
    ...
    const processedFirstRow: Record<string, null | number | string | Uint8Array> = {};
  ...
  filter(predicate: (value: never, index: number, array: never[]) => boolean): never[] {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (typeof sqlValue === 'object' && 'Integer' in sqlValue) {
          ...
          throw new TypeError('Only expected Integer, Real, Text, Blob, or Null');
        }
      }
  ...
  async execute(sql: string | Statement) {
    if (typeof sql === 'string') {
      ...
      if (Object.keys(sql.params).length === 0) {
        affectedRows = await op_execute_application_sql(this.name, sql.statement);
      } else {
        ...
        const paramListId = prepareParamList(paramKeys.map((key) => sql.params[key]));

        affectedRows = await op_execute_application_sql(this.name, preparedStatement, paramListId);
      }
  ...
  async query(sql: string | Statement): Promise<Rows> {
    ...
    if (typeof sql === 'string') {
      ...
      if (Object.keys(sql.params).length === 0) {
        ...
        result = await op_query_application_sql(this.name, preparedStatement, paramListId);
      }
    } else {
      throw new TypeError('Expected `sql` to be a string or Sql object');
    }
...
class NftSqlStore implements SqlNftDatabase {
  ...
  async execute(
    ...
    if (typeof sql === 'string') {
      result = await op_execute_nft_sql(this.name, resourceAddress, prepareNftId(nftId), sql);
    } else if (sql instanceof Statement) {
      if (Object.keys(sql.params).length === 0) {
  ...
  async query(
    ...
    if (typeof sql === 'string') {
      result = await op_query_nft_sql(this.name, resourceAddress, prepareNftId(nftId), sql);
    } else if (sql instanceof Statement) {
      if (Object.keys(sql.params).length === 0) {
  ...
  async execute(sql: string | Statement) {
    ...
    if (typeof sql === 'string') {
      ...
      if (Object.keys(sql.params).length === 0) {
        ...
        result = await op_execute_personal_sql(this.name, preparedStatement, paramListId);
      }
    } else {
      throw new TypeError('Expected `sql` to be a string or Sql object');
    }
  ...
  async query(sql: string | Statement): Promise<Rows> {
    ...
    if (typeof sql === 'string') {
      ...
      if (Object.keys(sql.params).length === 0) {
        ...
        result = await op_query_personal_sql(this.name, preparedStatement, paramListId);
      }
    } else {
      throw new TypeError('Expected `sql` to be a string or Sql object');
    }
