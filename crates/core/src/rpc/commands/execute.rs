...
impl ExecuteCommand {
    ...
    async fn create_code_package_from_manifest(manifest: &BundleManifest) -> Result<CodePackage, String> {
        use std::collections::HashMap;
        ...
        if module_roots.is_empty() && !manifest.modules.is_empty() {
            let first_module_url = if manifest.modules[0].path.starts_with("file://") {
                ...
                format!("file:///{}", manifest.modules[0].path.trim_start_matches('/'))
            };
        ...
        let package_json = if !manifest.dependencies.is_empty() {
            ...
            Some(package_json_str.parse::<PackageJson>()
