...
pub struct LogBatch {
    ...
    pub entries: Vec<(LogIndex, Bytes)>,
...
mod tests {
    ...
    async fn test_batching_by_size() {
        ...
        let entries1 = vec![(LogIndex::new(1).unwrap(), Bytes::from(large_data.clone()))];
        let entries2 = vec![(LogIndex::new(2).unwrap(), Bytes::from(large_data))];
