...
use crate::core::global::global_state::{
    ConsensusGroupInfo, GlobalState, MessageData as InternalMessageData,
    StreamData as InternalStreamData,
};
...
pub struct SnapshotData {
    ...
    /// Consensus groups
    pub consensus_groups: HashMap<ConsensusGroupId, ConsensusGroupInfo>,
...
impl SnapshotData {
    ...
    pub async fn from_global_state(global_state: &GlobalState) -> Self {
        ...
        SnapshotData {
            ...
            consensus_groups: consensus_groups.clone(),
        }
    }
    ...
    pub async fn restore_to_global_state(&self, global_state: &GlobalState) {
                ...
                let restored_messages = snapshot_data
                    ...
                    .map(|(seq, snapshot_msg)| {
                        ...
                        let msg = InternalMessageData {
                            ...
                            source: crate::core::global::global_state::MessageSource::Consensus,
                        };
                        (*seq, msg)
                    })
                    .collect();
        ...
        // Restore consensus groups
        {
            let mut consensus_groups = global_state.consensus_groups.write().await;
            consensus_groups.clear();
            for (group_id, group_info) in &self.consensus_groups {
                consensus_groups.insert(*group_id, group_info.clone());
            }
        }
...
//                     initial_members: vec![proven_topology::NodeId::from_seed(1)],
//                 }),
//                 1,
//             )
//             .await;
...
//                     config: crate::storage::stream::StreamConfig::default(),
//                     group_id: crate::ConsensusGroupId::new(1),
//                 }),
//                 2,
//             )
