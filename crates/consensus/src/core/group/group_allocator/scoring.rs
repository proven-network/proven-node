...
pub struct ScoringConfig {
    ...
    /// Weight for network latency optimization (0.0 - 1.0)
    pub network_latency_weight: f64,
    /// Weight for rack diversity (0.0 - 1.0)
    pub rack_diversity_weight: f64,
}

impl Default for ScoringConfig {
    fn default() -> Self {
        Self {
            az_diversity_weight: 0.3,
            size_balance_weight: 0.2,
            load_balance_weight: 0.15,
            membership_weight: 0.1,
            network_latency_weight: 0.15,
            rack_diversity_weight: 0.1,
        }
    }
}
...
pub struct AllocationScore {
    ...
    /// Network latency score (0.0 = poor, 1.0 = excellent)
    pub network_latency_score: f64,
    /// Rack diversity score (0.0 = poor, 1.0 = excellent)
    pub rack_diversity_score: f64,
...
impl AllocationScorer {
        ...
        G: Governance,
    {
        ...
        let node = match topology_manager.get_governance_node(node_id).await? {
            ...
            None => {
                return Ok(AllocationScore {
                    ...
                    network_latency_score: 0.0,
                    rack_diversity_score: 0.0,
                    total_score: 0.0,
                });
            }
        };
        ...
        let network_latency_score = self
            .calculate_network_latency_score(node_id, group, topology_manager)
            .await?;
        let rack_diversity_score = self
            .calculate_rack_diversity_score(node_id, group, topology_manager)
            .await?;
        ...
        let total_score = (az_diversity * self.config.az_diversity_weight)
            ...
            + (network_latency_score * self.config.network_latency_weight)
            + (rack_diversity_score * self.config.rack_diversity_weight);
        ...
        Ok(AllocationScore {
            ...
            load_balance,
            membership_score,
            network_latency_score,
            rack_diversity_score,
            total_score,
        })
    }
    ...
    /// Calculate network latency score based on proximity to existing group members
    async fn calculate_network_latency_score<G>(
        &self,
        node_id: &NodeId,
        group: &ConsensusGroup,
        topology_manager: &TopologyManager<G>,
        ...
        G: Governance,
    {
        if group.members.is_empty() {
            return Ok(1.0); // Perfect score for first member
        }
        ...
        // Calculate average "distance" to existing members
        let mut total_distance = 0.0;
        let mut member_count = 0;

        for member_id in &group.members {
            if let Some(member) = topology_manager.get_governance_node(member_id).await? {
                // Simple distance metric based on AZ and region
                let distance = if member.region != node.region {
                    3.0 // Different region = high latency
                } else if member.availability_zone != node.availability_zone {
                    1.0 // Different AZ = medium latency
                } else {
                    0.0 // Same AZ = low latency
                };
        ...
        // Convert average distance to score (lower distance = higher score)
        let avg_distance = total_distance / member_count as f64;
        let score = match avg_distance {
            d if d <= 0.5 => 1.0,
            d if d <= 1.0 => 0.8,
            d if d <= 2.0 => 0.5,
            _ => 0.2,
    ...
    /// Calculate rack diversity score
    async fn calculate_rack_diversity_score<G>(
        &self,
        node_id: &NodeId,
        group: &ConsensusGroup,
        topology_manager: &TopologyManager<G>,
        ...
        G: Governance,
    {
        ...
        // For now, use AZ as a proxy for rack diversity since rack info is not available
        // In a real deployment, rack information would be part of the governance node
        let node_rack = &node.availability_zone;

        // Count rack distribution in the group
        let mut rack_counts: HashMap<String, usize> = HashMap::new();
        for member_id in &group.members {
            if let Some(member) = topology_manager.get_governance_node(member_id).await? {
                // Use AZ as proxy for rack
                *rack_counts
                    .entry(member.availability_zone.clone())
                    .or_insert(0) += 1;
            }
        ...
        // Score based on rack diversity
        if rack_counts.is_empty() {
            return Ok(1.0); // First member
        }

        let is_new_rack = !rack_counts.contains_key(node_rack);
        let rack_diversity = rack_counts.len() as f64 / (rack_counts.len() + 1) as f64;

        // Bonus for adding a new rack
        let new_rack_bonus = if is_new_rack { 0.3 } else { 0.0 };

        // Penalty for overloading a rack
        let current_count = rack_counts.get(node_rack).copied().unwrap_or(0);
        let members_per_rack = (group.members.len() + 1) as f64
            / (rack_counts.len() + if is_new_rack { 1 } else { 0 }) as f64;
        let overload_penalty = if current_count as f64 + 1.0 > members_per_rack * 1.5 {
            0.2
        } else {
            0.0
        };

        Ok((rack_diversity + new_rack_bonus - overload_penalty).clamp(0.0, 1.0))
    }
}

#[cfg(test)]
mod tests {
    use crate::core::group::group_allocator::allocator::GroupState;
    ...
    async fn test_az_diversity_scoring() {
        ...
        let topology_manager =
            proven_topology::TopologyManager::new(governance, NodeId::from_seed(0))
                .await
                .unwrap();
