...
use crate::{
    ...
    core::stream::{
        ...
        storage::log_types::{MessageMetadata as StreamLogMetadata, MessageSource, StreamLogEntry},
    },
    storage_backends::{StorageEngine, StorageIterator, StorageKey, StorageValue, keys},
};
use bytes::Bytes;
...
/// Stream metadata stored in local state machine
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocalStreamMetadata {
...
impl StorageBackedLocalState {
    ...
    pub fn stream_manager(&self) -> &Arc<UnifiedStreamManager> {
        &self.stream_manager
    }
    ...
    pub async fn get_stream_metadata(
        &self,
        stream_id: &str,
    ) -> Result<Option<LocalStreamMetadata>, String> {
        ...
        {
            Some(value) => {
                let metadata: LocalStreamMetadata = ciborium::from_reader(value.as_bytes())
    ...
    pub async fn create_stream(&self, stream_id: &str, config: StreamConfig) -> Result<(), String> {
        ...
        let metadata = LocalStreamMetadata {
            last_seq: 0,
            is_paused: false,
            paused_at: None,
            message_count: 0,
    ...
    pub async fn publish_message(
        ...
        let storage = self
            ...
            .get_stream_storage(stream_id)
            .await
            .map_err(|e| format!("Stream {stream_id} not found: {e}"))?;
        ...
        let log_metadata = LocalStreamMetadata {
            last_seq: sequence,
            is_paused: false,
            paused_at: None,
            message_count: metadata.message_count + 1,
            total_bytes: metadata.total_bytes + data.len() as u64,
        };
    ...
    pub async fn get_message(
        ...
        match storage
            .get(&namespaces::messages(), &key)
            .await
            .map_err(|e| format!("Storage error: {e}"))?
        {
            Some(value) => {
                // Deserialize the StreamLogEntry
                let log_entry: StreamLogEntry = ciborium::from_reader(value.as_bytes())
                    .map_err(|e| format!("Failed to deserialize log entry: {e}"))?;
                Ok(Some(log_entry.data))
            }
            None => Ok(None),
        }
    }
    ...
    pub async fn create_checkpoint_metadata(
        &self,
        stream_id: &str,
    ) -> Result<crate::core::group::migration::MigrationCheckpoint, String> {
        let storage = self
            .stream_manager
            .get_stream_storage(stream_id)
            .await
        ...
        let storage_type = match storage.storage_type() {
            "memory" => crate::core::group::migration::StorageType::Memory,
            "rocksdb" => crate::core::group::migration::StorageType::RocksDB,
            _ => crate::core::group::migration::StorageType::Memory, // Default fallback
        };
        ...
        let stream_config = StreamConfig {
            ...
            storage_type: match storage_type {
                crate::core::group::migration::StorageType::Memory => {
                    crate::config::stream::StorageType::Memory
                }
                crate::core::group::migration::StorageType::RocksDB => {
                    crate::config::stream::StorageType::File
                }
                crate::core::group::migration::StorageType::S3 => {
                    crate::config::stream::StorageType::File
                }
            },
        ...
        Ok(crate::core::group::migration::MigrationCheckpoint {
            ...
            compression: crate::core::group::migration::CompressionType::None,
            is_incremental: false,
            base_checkpoint_seq: None,
            message_count: metadata.message_count,
            total_bytes: metadata.total_bytes,
            stream_metadata: crate::core::group::migration::MigrationStreamMetadata {
    ...
    pub async fn create_export_iterator(
        ...
        start_seq: u64,
        end_seq: u64,
    ) -> Result<crate::core::stream::storage::migration_iterator::MigrationMessageIterator, String>
    {
        use crate::core::stream::storage::migration_iterator::MigrationMessageIterator;
    ...
    pub async fn prepare_checkpoint_apply(
        &mut self,
        checkpoint: &crate::core::group::migration::MigrationCheckpoint,
    ) -> Result<(), String> {
    ...
    pub async fn apply_message_batch(
        &mut self,
        stream_name: &str,
        messages: Vec<crate::core::group::migration::StreamMessage>,
    ) -> Result<(), String> {
        let storage = self
            .stream_manager
        ...
        let mut batch = crate::storage_backends::WriteBatch::new();
        ...
        for msg in messages {
            ...
            let log_entry = StreamLogEntry {
                ...
                metadata: StreamLogMetadata {
                    headers: msg.headers,
                    compression: None, // Compression info not in StreamMessage
                    source: match msg.source {
                        crate::core::group::migration::MessageSourceType::Consensus => {
                            MessageSource::Consensus
                        }
                        crate::core::group::migration::MessageSourceType::PubSub {
                            subject,
                            publisher,
                        } => MessageSource::PubSub { subject, publisher },
                        crate::core::group::migration::MessageSourceType::Migration {
                            source_group,
                        } => MessageSource::Migration {
    ...
    /// Get metrics
    pub async fn get_metrics(&self) -> crate::core::group::LocalStateMetrics {
        ...
        for stream_name in stream_list {
            if let Ok(Some(metadata)) = self.get_stream_metadata(&stream_name).await {
                stream_metrics.insert(
                    stream_name,
                    crate::core::group::StreamMetrics {
                        message_count: metadata.message_count,
        ...
        crate::core::group::LocalStateMetrics {
