...
use crate::local::stream_storage::traits::{
    CheckpointFormat, CompressionType, MessageData, StorageType, StreamConfig,
    StreamExport as TraitsStreamExport, StreamMetrics, StreamStorage,
};
use crate::migration::{
    CompressionType as MigrationCompressionType, MigrationCheckpoint,
    StreamMetadata as MigrationStreamMetadata,
};
use crate::storage::{
...
mod keys {
    ...
    pub fn stream_message(stream_name: &str, sequence: u64) -> StorageKey {
        StorageKey::from(format!("msg_{}_{:016x}", stream_name, sequence).as_str())
    }
...
impl StreamManager {
    ...
    pub async fn append_message_with_hints(
        ...
        let hints = match delivery_guarantee {
            DeliveryGuarantee::AtLeastOnce => StorageHints {
                is_batch_write: true,
                access_pattern: crate::storage::traits::AccessPattern::Sequential,
    ...
    pub async fn create_stream_with_config(
        ...
        let metadata = crate::local::stream_storage::traits::StreamMetadata {
    ...
    async fn get_metadata(
        &self,
        stream_name: &str,
    ) -> ConsensusResult<crate::local::stream_storage::traits::StreamMetadata> {
    ...
    async fn create_checkpoint(
        ...
        Ok(MigrationCheckpoint {
            ...
            storage_type: match metadata.config.storage_type {
                StorageType::Memory => crate::migration::StorageType::Memory,
                StorageType::File => crate::migration::StorageType::RocksDB,
            },
