...
//! storage backends for stream data, similar to GlobalStorageType and UnifiedLocalStorage.

use crate::storage::{
    StorageIterator,
    adaptors::{memory::MemoryStorage, rocksdb::RocksDBStorage},
    traits::{
...
pub enum UnifiedStreamIterator {
    ...
    Memory(crate::storage::adaptors::memory::MemoryIterator),
    /// RocksDB storage iterator (also returns MemoryIterator after collecting)
    RocksDB(crate::storage::adaptors::memory::MemoryIterator),
}
...
impl UnifiedStreamStorage {
    ...
    pub async fn rocksdb(
        config: crate::storage::adaptors::rocksdb::RocksDBAdaptorConfig,
    ) -> StorageResult<Self> {
...
impl UnifiedStreamStorage {
    ...
    pub async fn append_stream_entry(
        &self,
        namespace: &StorageNamespace,
        entry: crate::storage::log::LogEntry<
            crate::local::stream_storage::log_types::StreamMetadata,
        >,
    ) -> StorageResult<()> {
        match self {
            Self::Memory(storage) => {
    ...
    pub async fn get_stream_entry(
        ...
        Option<
            crate::storage::log::LogEntry<crate::local::stream_storage::log_types::StreamMetadata>,
        >,
    > {
        match self {
            Self::Memory(storage) => {
    ...
    pub async fn read_stream_range<R: RangeBounds<u64> + Send>(
        ...
        Vec<crate::storage::log::LogEntry<crate::local::stream_storage::log_types::StreamMetadata>>,
    > {
        match self {
            Self::Memory(storage) => {
                use crate::storage::log::LogStorage;
    ...
    pub async fn get_stream_log_state(
        ...
        match self {
            Self::Memory(storage) => {
                use crate::storage::log::LogStorage;
                <_ as LogStorage<crate::local::stream_storage::log_types::StreamMetadata>>::get_log_state(storage.as_ref(), namespace).await
            }
            Self::RocksDB(storage) => {
                use crate::storage::log::LogStorage;
                <_ as LogStorage<crate::local::stream_storage::log_types::StreamMetadata>>::get_log_state(storage.as_ref(), namespace).await
    ...
    pub async fn truncate_stream_log(
        ...
        match self {
            Self::Memory(storage) => {
                use crate::storage::log::LogStorage;
                <_ as LogStorage<crate::local::stream_storage::log_types::StreamMetadata>>::truncate(storage.as_ref(), namespace, after_index).await
            }
            Self::RocksDB(storage) => {
                use crate::storage::log::LogStorage;
                <_ as LogStorage<crate::local::stream_storage::log_types::StreamMetadata>>::truncate(storage.as_ref(), namespace, after_index).await
