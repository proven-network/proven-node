...
mod keys {
    ...
    pub fn stream_entry(stream_id: &str, index: u64) -> StorageKey {
        StorageKey::from(format!("stream_{}_{:016x}", stream_id, index).as_str())
    }
...
impl<S: StorageEngine + LogStorage<LocalLogMetadata>> LocalStorage<S> {
    ...
    pub async fn read_stream_batch(
        ...
        let keys: Vec<crate::storage::StorageKey> = indices
            .iter()
    ...
    pub async fn append_stream_batch(
        ...
        for entry in entries {
            ...
            let value = crate::storage::StorageValue::new(serialize(&entry)?);
    ...
    pub async fn check_health(&self) -> GroupHealthStatus {
        ...
        if let Some(latency) = stats.avg_read_latency_ms {
            if latency > 50.0 {
                ...
                return GroupHealthStatus::Degraded(format!("High read latency: {:.2}ms", latency));
            }
        }
