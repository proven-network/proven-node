...
impl StorageEngine for S3StorageAdaptor {
    ...
    async fn put_with_hints(
        ...
        match hints.access_pattern {
            crate::storage_backends::traits::AccessPattern::WriteOnce => {
            ...
            crate::storage_backends::traits::AccessPattern::Temporary => {
                // Skip WAL for temporary data with low priority
                if hints.priority == crate::storage_backends::traits::Priority::Low {
    ...
    async fn write_batch(&self, batch: WriteBatch) -> StorageResult<()> {
        use crate::storage_backends::types::BatchOperation;
        ...
        if !deletes.is_empty() {
            ...
            for chunk in deletes.chunks(1000) {
                ...
                for (namespace, key) in chunk {
                    ...
                    delete_objects = delete_objects.objects(
                        aws_sdk_s3::types::ObjectIdentifier::builder()
                            ...
                            .build()
                            .map_err(|e| {
                                StorageError::Backend(format!("Failed to build delete object: {e}"))
                            })?,
                    );
                }
                ...
                self.client
                    ...
                    .await
                    .map_err(|e| {
                        StorageError::Io(std::io::Error::other(format!("Batch delete failed: {e}")))
                    })?;
            }
        }
        ...
        if !puts.is_empty() {
            for (namespace, key, value) in puts {
                ...
                let entry = super::batching::BatchEntry {
                    s3_key,
                    data: value.0,
                    wal_id: super::wal_client::WalEntryId {
                        id: uuid::Uuid::new_v4(),
                        offset: 0,
                    },
                };
...
impl SnapshotStorage for S3StorageAdaptor {
    async fn create_snapshot(
        ...
        loop {
            ...
            let response = request.send().await.map_err(|e| {
                StorageError::Io(std::io::Error::other(format!(
                    "Failed to list objects for snapshot: {e}"
                )))
            })?;
    ...
    async fn snapshot_metadata(
        ...
        let response = self
            ...
            .map_err(|e| {
                StorageError::Io(std::io::Error::other(format!(
                    "Failed to get snapshot metadata: {e}"
                )))
            })?;
