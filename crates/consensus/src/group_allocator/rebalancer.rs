...
impl GroupRebalancer {
    ...
    pub async fn generate_plan<G>(
        &self,
        topology_manager: &crate::topology::TopologyManager<G>,
                        ...
                        for node_id in nodes_to_move {
                                ...
                                .await?
                            {
                                migrations.push(NodeMigration {
                                    ...
                                    priority: 0.7,
                                    reason: format!(
                                        "Rebalancing AZ {} distribution in group {:?}",
                                        az, group_id
                                    ),
                                });
                            }
        ...
        G: proven_governance::Governance + Send + Sync + 'static,
    {
        ...
        let group = allocations
            .groups
            .get(&group_id)
            .ok_or_else(|| Error::InvalidOperation(format!("Group {:?} not found", group_id)))?;
    ...
    pub fn update_migration_phase(
        ...
        let state = self
            ...
            .get_mut(node_id)
            .ok_or_else(|| {
                Error::InvalidOperation(format!("Node {:?} is not migrating", node_id))
            })?;
