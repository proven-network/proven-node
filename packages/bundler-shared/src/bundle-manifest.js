...
exports.BundleManifestUtils = exports.BundleManifestGenerator = void 0;
...
class BundleManifestGenerator {
    constructor(projectRoot, options = {}) {
        ...
        this.packageAnalysis = new package_analysis_1.PackageAnalysis(projectRoot);
        this.fileCollection = new file_collection_1.FileCollection(projectRoot, options);
    }
    /**
    ...
    async generateManifest() {
        ...
        const dependencies = await this.packageAnalysis.analyzeDependencies(this.options.includeDevDependencies);
        console.log('Discovering entrypoints...');
        const entrypoints = await this.fileCollection.discoverEntrypoints();
        if (entrypoints.length === 0) {
            throw new Error('No entrypoints found. Make sure your project contains files that import @proven-network/handler');
        }
    ...
    async validateManifest(manifest) {
        ...
        const depValidationErrors = await this.packageAnalysis.validateDependencies(manifest.dependencies.production);
     ...
     */
    generateMetadata(sources, entrypoints) {
     ...
     */
    detectCircularDependencies(entrypoints, sources) {
        const circular = [];
        ...
        for (const entrypoint of entrypoints) {
            ...
            adjacencyList.set(entrypoint.filePath, deps.filter(Boolean));
        }
     ...
     */
    resolveImportToSourcePath(importPath, fromFile, sources) {
...
exports.BundleManifestGenerator = BundleManifestGenerator;
/**
...
class BundleManifestUtils {
     ...
     */
    static calculateStats(manifest) {
        const stats = {
     ...
     */
    static validateFormat(manifest) {
     ...
     */
    static extractHandlerDocs(manifest) {
        const handlers = [];
        for (const entrypoint of manifest.entrypoints) {
            for (const handler of entrypoint.handlers) {
                handlers.push({
...
exports.BundleManifestUtils = BundleManifestUtils;
