  ...
  describe("Version Management", function () {
    ...
    it("Should activate a version via governance", async function () {
      ...
      const activateVersionCalldata =
        versionGovernance.interface.encodeFunctionData("activateVersion", [
          sequence,
        ]);
    ...
    it("Should deactivate a version via governance", async function () {
      ...
      // Then deactivate it
      const deactivateVersionCalldata =
        versionGovernance.interface.encodeFunctionData("deactivateVersion", [
          sequence,
        ]);
    ...
    it("Should not allow activating a non-existent version", async function () {
      ...
      expect(currentState).to.be.oneOf([
    ...
    it("Should get all active versions correctly", async function () {
      ...
      for (let i = 1; i <= 3; i++) {
        const addVersionCalldata =
          versionGovernance.interface.encodeFunctionData("addVersion", [
            i,
            `pcr0-hash-${i}`,
            `pcr1-hash-${i}`,
            `pcr2-hash-${i}`,
